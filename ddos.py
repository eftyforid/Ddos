...
def main():
	...
	print ( Fore.RED + " Strom version 1.1 ")
	print ( Fore.RED + Style.BRIGHT + "■□■□■" + Fore.GREEN + Style.BRIGHT +" By EFTY FORID " + Fore.RED + Style.BRIGHT + "■□■□■" + Fore.GREEN + Style.BRIGHT)
	print(Style.BRIGHT)
	a = 0
	url = None
	url_check_status = None
	thrnom = None
	url = input( Style.BRIGHT + "root" + Fore.RED + "@" + Fore.GREEN + "strom (URL)" + Fore.YELLOW + " ► " + Fore.RED )
	...
	thrnom = input( Style.BRIGHT + "root" + Fore.RED + "@" + Fore.GREEN + "strom (Threads)" + Fore.YELLOW + " ► " + Fore.RED )
	...
	if thrnom == "0":
		...
		print( Fore.RED + "Threads count is incorrect")
		restart_c = 6
		for y in range(int(5)):
			y += 1 
			restart_c -= 1
	...
	print( Style.BRIGHT + Fore.YELLOW +" ► " + Fore.GREEN + "OK" )
	print()
	print( Fore.YELLOW + Style.BRIGHT +"[" + Fore.RED + "1" + Fore.YELLOW + "]" + Style.BRIGHT + Fore.GREEN + " Use always one User-Agent" )
	print( Fore.YELLOW + Style.BRIGHT +"[" + Fore.RED + "2" + Fore.YELLOW + "]" + Style.BRIGHT + Fore.GREEN + " Changing User-Agent for each cycle" )
	print()
	mode_ask = input( Style.BRIGHT + "root" + Fore.RED + "@" + Fore.GREEN + "strom (Option)" + Fore.YELLOW + " ► " + Fore.RED )
	...
	if mode_ask == "1":
		...
		print( Style.BRIGHT + Fore.RED + "Invalid answer")
		restart_c = 6
		for y in range(int(5)):
			y += 1 
			restart_c -= 1
			print("Restart after " + str(restart_c))
			time.sleep(1)
		main()
	...
	print()
	ask_for_start = input( Style.BRIGHT + "root" + Fore.RED + "@" + Fore.GREEN + "strom (Start attack?)" + Fore.YELLOW + " ► " + Fore.RED )
			...
			try:
				...
				for i in range(int(thrnom)):
					...
					if url_check_status.status_code == 200:
						sys.stdout.write( Style.BRIGHT + Fore.GREEN + "[+] Attack is running! URL status code: " + str(url_check_status) + "\n")
					else:
						sys.stdout.write( Style.BRIGHT + Fore.GREEN + "[+] Attack is running! URL status code: " + str(url_check_status) + "\n")
			except:
				continue
			...
			try:
				...
				for i in range(int(thrnom)):
					...
					if url_check_status.status_code == 200:
						...
						sys.stdout.write( Style.BRIGHT + Fore.RED + "[+] Attack is running! URL status code: " + str(url_check_status) + "\n")
			except:
				continue
	...
	if ask_for_start == "NO" or ask_for_start == "N":
		print( Fore.RED + "Attack was canceled!" + Fore.RED )
		restart_c = 6
		for y in range(int(5)):
			y += 1 
			restart_c -= 1
